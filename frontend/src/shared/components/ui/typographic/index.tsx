 ...
 */
export function LinearHeading({ children, level }: SharedProps & { level: 1 | 2 | 3 | 4 | 5 | 6 }) {
  if (level === 1)
    return (
  ...
  return <h3 className={"scroll-m-20 my-4 py-2 text-2xl font-semibold tracking-tight"}>{children}</h3>;
}
...
export function LinearList({ children, variant }: SharedProps & { variant: "numeric" | "figured" }) {
  if (variant === "numeric") return <ol className="ml-6 list-decimal [&>li]:mt-2 marker:text-primary">{children}</ol>;
  else return <ul className="ml-6 list-disc [&>li]:mt-2 marker:text-primary">{children}</ul>;
}
